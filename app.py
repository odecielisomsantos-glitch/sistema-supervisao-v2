...
# --- PORTAL ---
    ...
    with st.sidebar: 
        ...
        # CHAVE SELETORA DE MODO DE COR SOLICITADA
    ...
    # VISÃO GESTOR (Ajustada com Gráfico Clean e Tendência)
    if role in ["GESTOR", "GESTÃO"]:
        ...
        with tab_a:
            ...
            if op_sel:
                ...
                with ct: st.write(""); show_t = st.toggle("📊 Exibir Tendência", value=False)
                ...
                for m_name in sel_m:
                    ...
                    if any(mask):
                        ...
                        if show_t and len(yr[mask]) > 1:
                            z = np.polyfit(xr[mask], yr[mask], 1)
                            p = np.poly1d(z)
                            fig.add_trace(go.Scatter(x=xr[mask], y=p(xr[mask]), name=f"Tendência {m_name}", line=dict(dash='dash', width=2), opacity=0.5))
