...
# 2. DESIGN SYSTEM - ALTA NITIDEZ E PÓDIO 3D INTERATIVO
st.markdown("""
    <style>
    ...
    .podium-critical { border-top: 6px solid #EF4444; }
...
if not st.session_state.auth:
    ...
    # =================================================================
    # ÁREA DO GESTOR (PROFISSIONAL E INTERATIVO)
    # =================================================================
    if role in ["GESTOR", "GESTÃO"]:
        ...
        with tab_v:
            ...
            # --- PÓDIO 3D INTERATIVO (TOP 3 VERDE E FONTES GRANDES) ---
            st.markdown("---")
            sorted_perf = df_perf_final.sort_values("_RawMeta", ascending=False).reset_index(drop=True)
            top_3 = sorted_perf.head(3)
            bottom_3 = sorted_perf.tail(3).iloc[::-1]

            def render_podium_card(row, rank_type, medal_icon, main_color="#111827"):
                return f"""
            ...
            st.subheader("🥇 Pódio de Elite (Performance Verde)")
            c_gold = st.columns(3)
            icons, types = ["🥇", "🥈", "🥉"], ["gold", "silver", "bronze"]
            for i in range(3):
                if i < len(top_3):
                    with c_gold[i]: st.markdown(render_podium_card(top_3.iloc[i], types[i], icons[i], "#10B981"), unsafe_allow_html=True)

            st.markdown("<br>", unsafe_allow_html=True)
            st.subheader("🔻 Foco Necessário")
            c_low = st.columns(3)
            for i in range(3):
                if i < len(bottom_3):
                    with c_low[i]: st.markdown(render_podium_card(bottom_3.iloc[i], "critical", "📉", "#EF4444"), unsafe_allow_html=True)
        ...
        with tab_a:
            ...
            if op_sel:
                audit_data = df_h[df_h['Nome'].apply(norm).str.contains(norm(op_sel.split()[0]), na=False)].copy()
                sel_met = st.multiselect("Métricas:", audit_data['Métrica'].unique().tolist(), default=audit_data['Métrica'].unique().tolist())
                fig = go.Figure()
                for m_n in sel_met:
                    row = audit_data[audit_data['Métrica'] == m_n].iloc[0]
        ...
        with tab_gb:
            ...
            st.info("Página GB pronta para vinculação estratégica.")
        st.markdown('</div>', unsafe_allow_html=True)

    # =================================================================
    # ÁREA DO OPERADOR (RESTAURADA E BLINDADA)
    # =================================================================
    else:
        df_h = df_raw.iloc[26:211, 0:33].copy()
