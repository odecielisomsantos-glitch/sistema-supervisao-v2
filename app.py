...
# 2. Inicialização do Tema (Resolve o delay do clique e fontes)
if 'dark_mode' not in st.session_state:
    st.session_state.dark_mode = False

def update_theme():
    # Callback para atualização instantânea do tema
    pass

# Variáveis de Cores Adaptativas para Modo Noturno
...
st.markdown(f"""
    ...
    /* Botão Sair Premium */
    .logout-btn {{ background: #EF4444; color: white !important; border: none; padding: 5px 15px; border-radius: 6px; font-weight: bold; cursor: pointer; font-size: 11px; text-decoration: none; }}
    </style>
""", unsafe_allow_html=True)

# Função de Conexão com Tratamento de Erro para APIError
def get_data(aba):
    try:
...
# --- PORTAL PRINCIPAL ---
    ...
    # 3. Processamento de Dados (Intervalo AJ2:AT25)
    ...
    if df_raw is not None and df_rel is not None:
        ...
        # AJ=35, AT=45. Linha 2=cabeçalho (index 1)
        ...
        # 5. BARRA LARANJA COM SINO INTERATIVO E LUA
        st.markdown('<div class="nav-orange">', unsafe_allow_html=True)
        c_sino, c1, c2, c3, c_unid = st.columns([0.5, 1.5, 1.5, 1.5, 3])
        ...
        with c_sino:
            with st.popover("🔔", help="Clique para ver notificações"):
                ...
                st.write("---")
                st.info("Nenhuma nova notificação no momento.")
        ...
        with c_unid:
            ...
            with un2: st.toggle("🌙", value=st.session_state.dark_mode, on_change=update_theme, key="dark_toggle")
        ...
        with col_chart:
            st.markdown(f"### 📈 Histórico de Performance - {nome_oficial}")
            if not u_hist.empty:
                # Transpõe para gráfico: Datas (X) vs Valores (Y)
            ...
            else: st.warning(f"Dados não encontrados para '{primeiro_nome}' em RELATÓRIO AJ2:AT25.")
