...
# Variáveis de Cores Adaptativas para Modo Escuro
...
def get_data(aba):
    try:
        ...
        st.error(f"Erro ao conectar com a planilha: {aba}")
        return None
...
if not st.session_state.auth:
    ...
    if df_raw is not None and df_rel is not None:
        ...
        with c_sino:
            with st.popover("🔔", help="Informativo Geral"):
        ...
        with c_unid:
            ...
            with un2: st.toggle("🌙", value=st.session_state.dark_mode, on_change=toggle_theme, key="theme_tgl")
        ...
        for idx, row in rk.iterrows():
            ...
            with cols[idx % 8]:
                st.markdown(f'''<div class="card">{crown}<div class="av">{ini}</div><div style="font-size:10px;font-weight:700;height:35px;line-height:1.2;">{" ".join(str(row["Nome"]).split()[:2])}</div><div style="font-size:22px;font-weight:800;color:{color};">{row["Meta_Str"]}</div></div>''', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
