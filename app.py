...
# Inicialização e Callback para o Tema (Resolve o delay do clique)
...
colors = {
    ...
    "sub_text": "#9CA3AF" if is_dark else "#4B5563"
}

# 2. CSS Profissional Adaptativo
st.markdown(f"""
    <style>
    header, footer, #MainMenu {{visibility: hidden;}}
    .stApp {{ background: {colors['bg']}; font-family: 'Inter', sans-serif; transition: all 0.2s ease; }}
    
    /* Remoção da Sidebar e Ajuste de Fontes */
    [data-testid="stSidebar"] {{ display: none; }}
    .main .block-container {{ padding: 2rem 5rem; max-width: 100%; color: {colors['text']}; }}
    
    h1, h2, h3, p, span {{ color: {colors['text']} !important; }}
    ...
    .nav-label {{ font-size: 10px; text-transform: uppercase; opacity: 0.85; font-weight: 800; }}
    ...
    .card {{ 
        ...
        text-align: center; margin-bottom: 25px; height: 185px; 
    ...
    /* Botão Sair Premium */
    .logout-btn {{ background: #EF4444; color: white !important; border: none; padding: 6px 14px; border-radius: 6px; font-weight: 800; font-size: 11px; cursor: pointer; text-decoration: none; }}
    </style>
...
if not st.session_state.auth:
    ...
    # 3. Processamento de Dados (Foco: AJ2:AT25 do Relatório)
    ...
    # 5. BARRA LARANJA COM SINO E TOGGLE TEMA
    st.markdown('<div class="nav-orange">', unsafe_allow_html=True)
    c_sino, c1, c2, c3, c_unid = st.columns([0.6, 1.5, 1.5, 1.5, 3])
    
    with c_sino:
        with st.popover("🔔", help="Avisos do Gestor"):
            st.markdown(f"<h3 style='color:#111827; margin-bottom:10px;'>📢 Notificações</h3>", unsafe_allow_html=True)
            st.info("Sem novos avisos para hoje.")
    ...
    with c_unid:
        ...
        with uc2:
            st.toggle("🌙", value=st.session_state.dark_mode, on_change=toggle_theme, key="theme_toggle")

    st.markdown('</div>', unsafe_allow_html=True)

    st.markdown('<div class="main-content">', unsafe_allow_html=True)
    
    # 6. GRÁFICO E RANKING (50/50)
    ...
    with col_r:
        ...
        if not u_hist.empty:
            ...
            st.warning(f"Dados não localizados para '{primeiro_nome}' no intervalo AJ2:AT25.")
        ...
        with cols[idx % 8]:
            st.markdown(f'''
                <div class="card">
                    ...
                    <div style="font-size:10px;font-weight:700;height:35px;line-height:1.3;">{" ".join(str(row["Nome"]).split()[:2])}</div>
